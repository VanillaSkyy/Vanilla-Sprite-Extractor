...
def get_settings_path():
    if CONFIG_FILE.is_file():
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key.strip() == "azur_lane_path" and value.strip():
                    return Path(value.strip())
    
    path_str = input("Enter path to Azur Lane asset folder: ").strip()
    path = Path(path_str)
    ...
    try:
        ...
        lines = [line for line in lines if not line.lstrip().startswith("azur_lane_path")]
        lines.append(f"azur_lane_path = {path}\n")

        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
            f.writelines(lines)
        print(f"[INFO] Saved azur_lane_path to {CONFIG_FILE}")
...
def run(output_folder: Path):
    now = datetime.now()
    log_file = LOGS_DIR / f"azur_lane_{now.strftime('%H-%M-%S_%d-%m-%Y')}.txt"
