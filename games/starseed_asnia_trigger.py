...
def get_settings_path():
    if CONFIG_FILE.is_file():
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key.strip() == "starseed_path" and value.strip():
                    return Path(value.strip())
    
    path_str = input("Enter path to Starseed asset folder: ").strip()
    path = Path(path_str)
    ...
    try:
        ...
        lines = [line for line in lines if not line.lstrip().startswith("starseed_path")]
        lines.append(f"starseed_path = {path}\n")

        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
            f.writelines(lines)
        print(f"[INFO] Saved starseed_path to {CONFIG_FILE}")
...
def run(output_folder: Path):
    now = datetime.now()
    log_file = LOGS_DIR / f"starseed_asnia_trigger_{now.strftime('%H-%M-%S_%d-%m-%Y')}.txt"
