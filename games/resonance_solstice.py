...
decrypt_key = b'ResonanceRebornR'
...
def get_settings_path():
    if CONFIG_FILE.is_file():
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if key.strip() == "resonance_solstice_path" and value.strip():
                    return Path(value.strip())
    
    path_str = input("Enter path to Resonance Solstice asset folder: ").strip()
    path = Path(path_str)
    ...
    try:
        ...
        lines = [line for line in lines if not line.lstrip().startswith("resonance_solstice_path")]
        lines.append(f"resonance_solstice_path = {path}\n")

        with open(CONFIG_FILE, "w", encoding="utf-8") as f:
            f.writelines(lines)
        print(f"[INFO] Saved resonance_solstice_path to {CONFIG_FILE}")
...
def run(output_folder: Path):
    now = datetime.now()
    log_file = LOGS_DIR / f"resonance_solstice_{now.strftime('%H-%M-%S_%d-%m-%Y')}.txt"
